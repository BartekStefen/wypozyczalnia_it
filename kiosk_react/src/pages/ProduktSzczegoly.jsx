...
const CSS = `
  ...
  .stat-block { 
    ...
    padding: 20px; text-align: center; flex: 1; transition: 0.3s;
...
export default function ProduktSzczegoly() {
  ...
  const [isAdding, setIsAdding] = useState(false); // Zmienione z isBooking
  ...
  useEffect(() => {
    ...
    axios.get(`http://127.0.0.1:8000/api/sprzet/${id}`)
      .then(res => { setItem(res.data); setLoading(false); })
      .catch(() => setLoading(false));
    return () => document.head.removeChild(s);
  }, [id]);

  const dni = endDate ? differenceInDays(endDate, startDate) : 1;
  const suma = item ? (dni * item.cena_wypozyczenia_dzien).toFixed(2) : 0;

  // --- NOWA LOGIKA: DODAWANIE DO KOSZYKA ---
  const handleAddToCart = () => {
    if (!endDate) return alert("Wybierz datę zwrotu!");
    
    setIsAdding(true);

    // Pobieramy aktualny koszyk z pamięci przeglądarki
    ...
    // Zapisujemy i informujemy Navbar o zmianie
    ...
    setTimeout(() => {
      ...
      setIsAdding(false);
      navigate('/koszyk'); // Przenosimy użytkownika do koszyka
    }, 500);
  };
        ...
        <div className="col-lg-7">
          ...
          <div className="d-flex gap-3 mb-5">
            <div className="stat-block shadow-sm">
              ...
              <div className={`stat-value ${item.status === 'Dostępny' ? 'text-success' : 'text-danger'}`}>
                {item.status}
              </div>
            </div>
            <div className="stat-block shadow-sm">
          ...
          <div className="bg-white rounded-4 shadow-sm p-5 mb-5 border-0">
            ...
            <p className="text-secondary" style={{ lineHeight: '1.9', fontSize: '1.1rem' }}>
              ...
              Został poddany pełnej inspekcji technicznej, co gwarantuje płynność pracy przy montażu wideo, programowaniu czy projektowaniu.
              Klawiatura i ekran są w stanie nienagannym. System operacyjny jest czysty i gotowy do Twojej konfiguracji.
            </p>
          </div>
          ...
          <div className="bg-white rounded-4 shadow-sm p-5 border-0">
            ...
            <div className="review-card">
              ...
              <p className="small text-muted mb-0">"Używałem tego laptopa przez tydzień. Wszystko śmigało bez zająknięcia. Polecam!"</p>
            </div>
            ...
            <div className="mt-5 pt-4 border-top">
              ...
              <div className="d-flex gap-1 mb-3">
                {[1,2,3,4,5].map(s => (
                  <button key={s} className={`star-btn ${s <= rating ? 'active' : ''}`} onClick={() => setRating(s)}>★</button>
                ))}
              </div>
              <textarea 
                className="form-control mb-3 rounded-4 p-3 border-2" 
                rows="4" 
                placeholder="Podziel się swoimi wrażeniami..." 
                value={comment} 
                onChange={e => setComment(e.target.value)} 
              />
              <button className="btn btn-dark w-100 py-3 rounded-4 fw-bold shadow-sm">PUBLIKUJ RECENZJĘ</button>
            </div>
          </div>
        </div>
          ...
          <div className="sticky-top" style={{ top: '100px' }}>
            <div className="rent-card p-4">
              ...
              <div className="mb-4 text-center">
                ...
                <div 
                  className="calendar-trigger fw-bold text-primary mb-3"
              ...
              <div className="p-4 bg-light rounded-4 mb-4 border">
                <div className="d-flex justify-content-between mb-2 small text-muted">
                  ...
                  <span className="fw-bold text-dark">{dni} dni</span>
                </div>
              ...
              <button 
                className="btn btn-premium w-100 py-3 rounded-4 fw-bold shadow"
